...
/// Highest migration version this server knows, for the active backend.
fn current_schema_version() -> i64 {
    let migrator = match crate::db::kind() {
        crate::db::DbKind::Postgres => sqlx::migrate!("./migrations-pg"),
        crate::db::DbKind::Sqlite => sqlx::migrate!("./migrations"),
    };
    migrator.iter().map(|m| m.version).max().unwrap_or(0)
}

/// Connection parts for the `pg_dump`/`pg_restore` CLI tools, parsed from
/// the same `DATABASE_URL` the server uses. The password is passed via the
/// `PGPASSWORD` env var so it never shows up in the process list.
struct PgConn {
    host: String,
    port: u16,
    user: String,
...
fn pg_conn_info() -> Result<PgConn, ApiError> {
    let raw =
        std::env::var("DATABASE_URL").map_err(|_| ApiError::bad_request("DATABASE_URL unset"))?;
    let url = url::Url::parse(&raw)
        .map_err(|e| ApiError::bad_request(format!("bad DATABASE_URL: {e}")))?;
    let dbname = url.path().trim_start_matches('/').to_string();
    if dbname.is_empty() {
        return Err(ApiError::bad_request("DATABASE_URL has no database name"));
    }
    Ok(PgConn {
...
/// Build a `pg_dump`/`pg_restore` command targeting the server's database.
fn pg_tool(bin: &str, conn: &PgConn) -> std::process::Command {
...
fn pg_tool_err(bin: &str, e: std::io::Error) -> ApiError {
    ApiError {
        status: StatusCode::INTERNAL_SERVER_ERROR,
        message: format!("{bin} could not start: {e}"),
    }
}

fn pg_tool_failed(bin: &str, stderr: &[u8]) -> ApiError {
    ApiError {
        status: StatusCode::INTERNAL_SERVER_ERROR,
        message: format!("{bin} failed: {}", String::from_utf8_lossy(stderr).trim()),
    }
}
...
async fn download_backup(State(state): State<AppState>, user: CurrentUser) -> ApiResult<Response> {
    ...
    let db_dump = work_dir.join("crabcast.dump");
    let zip_path = work_dir.join("backup.zip");

    let (db_file, db_entry) = match crate::db::kind() {
        ...
        crate::db::DbKind::Postgres => {
            // Logical dump via pg_dump (custom format). Requires the
            // postgresql-client tools on the host.
            let conn = pg_conn_info()?;
            let dump_path = db_dump.clone();
            let mut cmd = pg_tool("pg_dump", &conn);
            cmd.args(["-Fc", "--no-owner", "--no-privileges"])
                .arg("-d")
                .arg(&conn.dbname)
                .arg("-f")
                .arg(&dump_path);
            let out = tokio::task::spawn_blocking(move || cmd.output())
                ...
                .map_err(|e| ApiError {
                    status: StatusCode::INTERNAL_SERVER_ERROR,
                    message: format!("pg_dump task panicked: {e}"),
                })?
                .map_err(|e| pg_tool_err("pg_dump", e))?;
            if !out.status.success() {
                return Err(pg_tool_failed("pg_dump", &out.stderr));
            }
            (db_dump, "crabcast.dump")
        }
    };

    build_backup(&zip_path, &db_file, &media_dir, &configs_dir, db_entry).await?;
...
async fn build_backup(
    ...
    let db_entry = db_entry.to_string();
    ...
    tokio::task::spawn_blocking(move || -> Result<Manifest, String> {
        ...
        zip.write_all(&serde_json::to_vec(&manifest).map_err(|e| e.to_string())?)
            .map_err(|e| e.to_string())?;
        add_file(&mut zip, method, &db_entry, &db_file)?;
...
async fn restore_backup(
    ...
    let db_entry = match crate::db::kind() {
        crate::db::DbKind::Postgres => "crabcast.dump",
        crate::db::DbKind::Sqlite => "crabcast.db",
    };
    let stage_for_validation = staging.clone();
    tokio::task::spawn_blocking(move || {
        stage_archive(&data, &stage_for_validation, current, db_entry)
    })
    .await
    .map_err(|e| ApiError {
        status: StatusCode::INTERNAL_SERVER_ERROR,
    ...
    if crate::db::kind() == crate::db::DbKind::Postgres {
        pg_restore_into_place(&staging, &ts).await?;
    } else {
...
fn stage_archive(
    ...
    for i in 0..archive.len() {
        ...
        let rel = archive_rel_path(&name, db_entry)
        ...
        if name == "crabcast.dump" {
            // pg_dump custom-format file: must be present and non-empty;
            // full integrity is checked with `pg_restore --list` on restore.
            let mut buf = Vec::new();
            entry.read_to_end(&mut buf).map_err(|e| e.to_string())?;
            if buf.is_empty() {
                return Err("crabcast.dump is empty".into());
            }
            std::fs::write(&dest, &buf).map_err(|e| e.to_string())?;
            continue;
        }
...
/// Only the active backend's DB artifact (`crabcast.db` or `crabcast.dump`),
/// `media/*` and `stations/configs/*` are accepted; any `..`, absolute or
/// otherwise suspicious component is rejected.
fn archive_rel_path(name: &str, db_entry: &str) -> Option<PathBuf> {
    if Path::new(name)
        .components()
...
/// Apply a staged Postgres dump to the live database: validate the dump
/// with `pg_restore --list` (no writes), keep a safety copy of the current
/// state, drop the live schemas, then restore. Runs only after the engines
/// are stopped; the caller exits for restart afterwards.
async fn pg_restore_into_place(staging: &std::path::Path, ts: &str) -> ApiResult<()> {
    let conn = pg_conn_info()?;
    let dump = staging.join("crabcast.dump");

    // Validate the archive's dump before touching the live DB.
    let mut list = pg_tool("pg_restore", &conn);
    list.arg("--list").arg(&dump);
    let out = tokio::task::spawn_blocking(move || list.output())
        .await
        .map_err(|e| ApiError {
            status: StatusCode::INTERNAL_SERVER_ERROR,
            message: format!("pg_restore task panicked: {e}"),
        })?
        .map_err(|e| pg_tool_err("pg_restore", e))?;
    if !out.status.success() {
        return Err(pg_tool_failed("pg_restore --list", &out.stderr));
    }

    // Safety copy of the current state, mirroring the SQLite
    // `.pre-restore-<ts>` swap-aside behaviour.
    let safety =
        std::env::temp_dir().join(format!("crabcast-{}.pre-restore-{ts}.dump", conn.dbname));
    let mut sdump = pg_tool("pg_dump", &conn);
    sdump
        ...
        .arg("-f")
        .arg(&safety);
    let out = tokio::task::spawn_blocking(move || sdump.output())
        .await
        .map_err(|e| ApiError {
            status: StatusCode::INTERNAL_SERVER_ERROR,
            message: format!("pg_dump task panicked: {e}"),
        ...
        .map_err(|e| pg_tool_err("pg_dump", e))?;
    if out.status.success() {
        tracing::info!("backup restore: current DB kept at {}", safety.display());
    } else {
        tracing::warn!(
            "backup restore: safety dump failed (continuing): {}",
            String::from_utf8_lossy(&out.stderr).trim()
        );
    }

    // pg_restore --clean drops and recreates every object in the dump, so no
    // manual schema surgery is needed; --if-exists keeps it idempotent.
    let mut restore = pg_tool("pg_restore", &conn);
    restore
        .args([
            "--clean",
            "--if-exists",
        ...
        .arg(&conn.dbname)
        .arg(&dump);
    let out = tokio::task::spawn_blocking(move || restore.output())
        .await
        .map_err(|e| ApiError {
            status: StatusCode::INTERNAL_SERVER_ERROR,
            message: format!("pg_restore task panicked: {e}"),
        ...
        .map_err(|e| pg_tool_err("pg_restore", e))?;
    if !out.status.success() {
        return Err(pg_tool_failed("pg_restore", &out.stderr));
    }
    tracing::info!("backup restore: Postgres dump applied");
    Ok(())
}
...
mod tests {
    ...
    fn archive_paths_are_sandboxed() {
        assert_eq!(
            archive_rel_path("crabcast.db", "crabcast.db"),
            Some(PathBuf::from("crabcast.db"))
        );
        assert_eq!(
            archive_rel_path("crabcast.dump", "crabcast.dump"),
            Some(PathBuf::from("crabcast.dump"))
        );
        assert_eq!(archive_rel_path("crabcast.db", "crabcast.dump"), None);
        assert_eq!(
            archive_rel_path("media/ab/abc.mp3", "crabcast.db"),
            Some(PathBuf::from("media/ab/abc.mp3"))
        );
        assert_eq!(
            archive_rel_path("stations/configs/s1/crabsoup.lua", "crabcast.db"),
            Some(PathBuf::from("configs/s1/crabsoup.lua"))
        );
        assert_eq!(archive_rel_path("manifest.json", "crabcast.db"), None);
        assert_eq!(archive_rel_path("media/../etc/passwd", "crabcast.db"), None);
        assert_eq!(archive_rel_path("../../etc/passwd", "crabcast.db"), None);
        assert_eq!(archive_rel_path("/etc/passwd", "crabcast.db"), None);
        assert_eq!(archive_rel_path("etc/passwd", "crabcast.db"), None);
    }
    ...
    fn stage_archive_extracts_and_rejects_bad_archives() {
        ...
        let parsed = stage_archive(&zip, &staging, 5, "crabcast.db").unwrap();
        ...
        assert!(stage_archive(&bad_db, &staging, 5, "crabcast.db").is_err());

        // A backup from a newer server is rejected, not silently applied.
        let too_new = make_zip(&manifest(9), b"SQLite format 3\0x", &[]);
        assert!(stage_archive(&too_new, &staging, 5, "crabcast.db").is_err());
        ...
        assert!(stage_archive(&buf, &staging, 5, "crabcast.db").is_err());
    }
}
